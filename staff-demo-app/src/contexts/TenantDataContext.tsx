...
import { getShowcaseTenantData, SHOWCASE_TENANT_ID } from '@/data/showcaseFixtures';
...
export function TenantDataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (!db) {
        if (IS_SHOWCASE_MODE && tenantId === SHOWCASE_TENANT_ID) {
          const fixtures = getShowcaseTenantData();
          if (mounted) {
            setData(fixtures);
            for (const { key, path } of COLLECTIONS) {
              const value = fixtures[key];
              if (value !== undefined) {
                await saveToCache(tenantId, path, value);
              }
            }
          }
        } else {
